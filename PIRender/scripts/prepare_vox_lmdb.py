...
def prepare_data(path, keypoint_path, coeff_3dmm_path, out, n_worker, sizes, chunksize, img_format):
    ...
    curr_3dmms = set([osp.basename(e)[:-4] for e in glob(f'{coeff_3dmm_path}/**/*.mat')])
    for item in filenames:
        basename = osp.basename(item)[:-4]
        if basename not in curr_3dmms:
            continue
        test_video.append(item)
    filenames = test_video
    print('Length of video:', len(train_video), len(test_video))
